...
import blackwallGatewayUrl from '../assets/tiles/BlackwallGateway.png?url'
...
/** Loaded texture for the Blackwall Gateway tile (§9.2). */
let gatewayTexture: Texture | null = null

/**
 * Load all tower textures and the Blackwall Gateway texture into the PixiJS asset cache.
...
export async function loadTowerTextures(): Promise<void> {
  await Promise.all([
    ...
    Assets.load<Texture>(blackwallGatewayUrl).then(tex => { gatewayTexture = tex }),
  ])
}
...
/**
 * Returns the PixiJS Texture for the Blackwall Gateway tile (§9.2).
 * Throws if loadTowerTextures() has not been called yet.
 */
export function getGatewayTexture(): Texture {
  if (!gatewayTexture) throw new Error('Gateway texture not loaded — call loadTowerTextures() first')
  return gatewayTexture
}
